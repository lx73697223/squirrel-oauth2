...
public class GrantByImplicitProviderTest extends OAuth2Test {
    ...
    @Test
    public void getJwtTokenByImplicitGrant()
        ...
        ResponseEntity<String> response = new TestRestTemplate("user", "password").postForEntity(
                "http://localhost:" + port +
                "oauth/authorize?response_type=token&client_id=normal-app&redirect_uri={redirectUrl}",
        ...
        response = new TestRestTemplate("user", "password").postForEntity(
                "http://localhost:" + port +
